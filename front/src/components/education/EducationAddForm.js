    ...
    <Form onSubmit={handleSubmit}>
      ...
      <div key={`inline-radio`} className="mb-3 mt-3">
        {positionList.map((object) => (
          // 여기서 화살표 함수가 나오는데 왜 => {} 중괄호를 써주지 않지? 라는 의문이 든다면 return을 기억하렴. arrow function의 유일한 문장이 return일 경우 return을 생략하는 게 가능한데 그럴 경우 중괄호도 날려야 한다. () => {return ~}일 필요가 없는 것. () => ()로 해도 된다.
          <Form.Check
            inline
            label={object.label}
            id={object.id}
